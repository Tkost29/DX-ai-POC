...
1. **情報取り込み（擬似）**: 3つのソースからの社内情報を統合
2. **ユーザー別優先表示**: 関連度スコアに基づく自動ソート
3. **AI要約生成**: 全体要約（5〜7行）+ 重要トピック箇条書き
4. **ユーザ別ビュー**: 部署・関心タグでフィルタリング
5. **アクション候補表示**: 要対応/共有推奨/参考情報ラベル

## 🖥️ 画面構成

- **`/dashboard`** - 情報統合ダッシュボード
- **`/priority`** - ユーザー別優先表示
- **`/summary`** - AI要約表示
- **`/actions`** - アクション示唆
